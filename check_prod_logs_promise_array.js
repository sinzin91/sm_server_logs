...
var _findWhichServer = (searchTerm, clb) => {
        ...
        if (stdout == 0){
          ...
          child_process.exec(searchQueryProd2, (error, stdout, stderr) => {
            ...
            if (stdout == 0) {
              ...
              _scanLogFile("prod2", stdout, searchTerm, clb);
            }
          })
        } else {
          console.log("The log file is in prod3.");
          _scanLogFile("prod3", stdout, searchTerm, clb);
        }
      });
    } else {
      console.log("The log file is in prod1.");
      _scanLogFile("prod1", stdout, searchTerm, clb);
    }
  });
}
var jsonOutputAggregate = [];
var _scanLogFile = (prodServer, stdout, searchTerm, clb) => {
...
var parser = parse({delimiter: ','}, function (err, data) {
  ...
  async.whilst(
    () => {console.log(`elmLeft: ${elmLeft}`); return (elmLeft > 0 && !flag)},
    (clb) => {
      arr = searchTerms.slice(pos, pos + MAX_CONCUR)

      async.some(arr,
        (elm, callback) => {
          _findWhichServer(elm, () => {
            callback(null, true);
          });
          // debugger
        },
        (err, result) => {
          ...
          if (result === true) {
            pos += MAX_CONCUR;
            elmLeft = searchTerms.length - pos;
          }
          else {
            flag = false;
          }

          clb(null, elmLeft, flag);
      }); // every
