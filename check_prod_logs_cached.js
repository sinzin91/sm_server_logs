...
var _findWhichServer = (searchTerm) => {
  var searchQueryProd1 = "ssh prod1 'sudo ls -lt /mint/logs/topologies | grep \"\\s" + searchTerm + ".*2016\"'";
  var searchQueryProd2 = "ssh prod2 'sudo ls -lt /mint/logs/topologies | grep \"\\s" + searchTerm + ".*2016\"'";
  var searchQueryProd3 = "ssh prod3 'sudo ls -lt /mint/logs/topologies | grep \"\\s" + searchTerm + ".*2016\"'";
   ...
   child_process.exec(searchQueryProd1, (error, stdout, stderr) => {
    if (stdout == 0){
      ...
      child_process.exec(searchQueryProd3, (error, stdout, stderr) => {
        if (stdout == 0){
          console.log("No results in prod3, checking prod2"); // check prod 3 first because it has more recent logs than prod2
          ...
          _scanLogFile("prod3", stdout);
        }
      });
    } else {
      console.log("The log file is in prod1.");
      _scanLogFile("prod1", stdout);
    }
  });
}

var _scanLogFile = (prodServer, stdout) => {
